...
const useDraggableContainer = ({
  ...
  const onDragStart = useCallback((parent: Container, container: Container) => {
  ...
  const onDragMove = useCallback((event: FederatedPointerEvent) => {
  ...
  const onDragEnd = useCallback((parent: Container) => {
    return () => {
  ...
  }, [parent, container, disabled, onDragStart, onDragMove]);
};
